...
esp_err_t sw_enableRx(SwSerial *self, bool State)
{
    ...
    if (State)
    {
         // ESP_LOGI("SW-ENABLE RX : ","%d",self->rxPin);
        ...
        //ESP_LOGI("SW-ENABLE ERROR : ","%s",esp_err_to_name(error));

    }
    else
    {
        ESP_LOGI("BURADAYIM : ","hataaaaa ======================");
